...
def status():
    ...
    return json.dumps({"time": datetime.now().strftime("%H:%M:%S"), "bateria": bat, "gps": gps, "key": KEY})
...
while True:
    try:
        ...
        if r.status_code == 200:
            ...
            salvar(nome, json.dumps({"cmd":cmd,"result":res,"time":datetime.now().strftime("%H:%M:%S")}))
